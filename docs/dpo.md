...
Mặc dù bạn có thể tiếp tục mở rộng SFT với nhiều dữ liệu hơn, đến một lúc nào đó bạn sẽ quan sát thấy diminishing returns (lợi ích giảm dần) hoặc failure mode (chế độ lỗi) — như mô hình không thể tự sửa code lỗi của mình. Tại sao?
...
**Direct Preference Optimization (DPO — Tối ưu hóa Sở thích Trực tiếp)** là thuật toán preference optimization đầu tiên được áp dụng rộng rãi trong open source.
