...
> [!WARNING]
> **Bài học quan trọng**: Luôn dùng **con số thực tế**, không phải marketing specs, khi lập kế hoạch training runs.
...
> [!NOTE]
> **TL;DR:** GPU trong một node có thể giao tiếp theo 3 cách: qua CPU (chậm nhất, ~3 GB/s), qua GPUDirect RDMA trên EFA NICs (~38 GB/s), hoặc qua NVLink (~786 GB/s bidirectional). NVLink nhanh hơn 9–112×.
...
> [!NOTE]
> **TL;DR:** Giao tiếp multi-node sử dụng mạng tốc độ cao như InfiniBand (400 Gbps) hoặc RoCE (100 Gbps). All-reduce scale tốt (320–350 GB/s, ổn định qua các node). All-to-all suy giảm mạnh hơn. Latency nhảy từ ~13 μs intranode lên 55 μs+ internode.
...
> [!NOTE]
> **TL;DR:** GPUDirect Storage (GDS) cho phép transfer trực tiếp GPU-to-storage, bypass CPU. Local NVMe RAID (8 × 3.5 TB ổ, RAID 0) cho **26.59 GiB/s** và **337k IOPS** — nhanh hơn 6.3× so với network storage.
...
$$N_{\text{GPU}} = \frac{\text{FLOPs}_{\text{total}}}{\text{Throughput}_{\text{GPU}} \times t_{\text{target}}}$$
